...
<template>
  <div>
    <label
      ...
      :id="props.labelFor"
      :class="[
        props.label ? 'block mb-2 text-medium font-semibold text-[#888EA8]' : 'hidden',
        { 'text-[#C0280C]': props.required && props.submitted }
      ]"
    >
      <slot name="label">{{ props.label }}<span v-if="props.required && props.submitted" class="ml-1">*</span></slot>
    </label>
    <div class="relative">
      ...
      <input
        ...
        class="border border-[#E0E6ED] min-h-[40px] py-2 px-3 text-black text-medium placeholder:text-white focus:outline-none focus:border-[#E0E6ED] focus:ring focus:ring-[#E0E6ED] block w-full disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-[#E0E6ED]"
        :class="{
